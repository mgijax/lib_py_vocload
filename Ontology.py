...
class OboOntology(DAG.DAG):
    ...
    # Return list of namespaces (list of strings)
        return self.namespaces.keys()

    def getRelationshipTypes(self):
    # Return list of RelationshipTypes (list of strings)
        return self.relationshipTypes.keys()
	...
	if type(term) is types.StringType:	# if term is ID
    ...
    # parent and child can be either IDs (strings) or OboTerms themselves
    # Assumes child and parent refer to existing OboTerms in the ontology.
	if type(parent) is types.StringType:	# if parent is ID
	    parent = self.getTerm(parent)
	if type(child) is types.StringType:	# if child is ID
	...
	if ns is None:
	    ...
	    return [r for sublist in self.nsRoots.values() for r in sublist]
	else:
...
class OboLoader(object):
	...
	for reln in stanza.get("relationship", []):
		...
		raise Exception("Unexpected relationship specification: " \
				+ str(tokens))
	    (rel,id2) = map(string.strip, tokens)
	...
	for attr, val in stanza.iteritems():	# set any other attrs for term
						#   from this stanza
